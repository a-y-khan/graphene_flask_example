...
from example.models import House as HouseModel
from example.models import Student as StudentModel
from example.models import Staff as StaffModel

import example.database as db
...
class StaffNode(gp_sa.SQLAlchemyObjectType):
    ...
    teacher = gp.String(description="A Hogwarts teacher.", deprecation_reason="Replaced with staff.")
    wand = gp.String(
        description="String that describes wand.",
...
class HouseEnrollment(gp.ObjectType):
    ...
    students = gp_sa.SQLAlchemyConnectionField(StudentConnection, description="Students sorted into this house.")
...
class Query(gp.ObjectType):
    ...
    all_houses = gp_sa.SQLAlchemyConnectionField(HouseConnection, description="Get all Hogwarts Houses.")
    all_students = gp_sa.SQLAlchemyConnectionField(StudentConnection,
                                                   description="Get all students enrolled at Hogwarts.")
    all_staff = gp_sa.SQLAlchemyConnectionField(StaffConnection, description="Get all staff who work at Hogwarts.")
    ...
    student_by_name = gp.Field(StudentNode, name=gp.String(), description="Get Hogwarts student by name.")
    staff_by_name = gp.Field(StaffNode, name=gp.String(), description="Get Hogwarts staff member by name.")
    house_by_name = gp.Field(HouseNode, name=gp.String(), description="Get Hogwarts House by name.")
    staff_by_position = gp.List(StaffNode, position=gp.String(), description="Get Hogwarts staff member by position.")
    ...
    def resolve_house_enrollment(self, info, house_name):
        ...
        house = house_query.filter(HouseModel.name == house_name).first()
        student_query = StudentNode.get_query(info)
        return HouseEnrollment(students=student_query.filter(StudentModel.house == house).all())
...
schema = gp.Schema(
    query=Query,
    types=[HouseNode, StudentNode, StaffNode, HouseEnrollment, SearchResult],
    mutation=Mutation,
)
